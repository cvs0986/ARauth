...
export const userApi = {
  ...
  getUserRoles: async (userId: string): Promise<Role[]> => {
    const response = await apiClient.get<Role[]>(`${API_ENDPOINTS.USERS}/${userId}/roles`);
    return response.data;
  },

  getUserPermissions: async (userId: string): Promise<Permission[]> => {
    const response = await apiClient.get<Permission[]>(`${API_ENDPOINTS.USERS}/${userId}/permissions`);
    return response.data;
  },
};
